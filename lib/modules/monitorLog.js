 ...
 * MIT Licensed
 */
var logger = require('@sex-pomelo/sex-pomelo-logger').getLogger('pomelo-admin', __filename);
...
var fetchLogs = function(root, msg, callback) {
	...
	exec('tail -n ' + number + ' ' + filePath, function(error, output) {
		...
		// console.log('---out:', output,output.length);
		...
		callback({logfile:logfile,data:output});
	});
};
